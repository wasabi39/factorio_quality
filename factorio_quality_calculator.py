...
def calculate_productivity_boost(number_of_productivity_chips=0, 
    ...
    if recycling:
        ...
        return -0.75
    prod_boost_from_chips = number_of_productivity_chips * get_productivity_chip_score()[type_of_productivity_chip - 1]
...
def generate_transition_matrix(computation_request: ComputationRequest):
    ...
    upper_right = get_part_of_transition_matrix(productivity_boost_for_assembly, quality_boost_for_assembly, recycling=False)
    ...
    lower_left = get_part_of_transition_matrix(productivity_boost_for_recycling, quality_boost_for_recycling, recycling=True)

    transition_matrix = concatenate_transition_matrices(upper_left, upper_right, lower_left, bottom_right)
...
def calculate_iterations(number_of_iterations: int, transition_matrix: np.array) -> ResultRequest:
    ...
    distribution = starting_distribution @ np.linalg.matrix_power(transition_matrix, 2 * number_of_iterations)
